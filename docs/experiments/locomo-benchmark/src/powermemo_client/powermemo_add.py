...
from powermemo import PowerMemoClient, AsyncPowerMemoClient, ChatBlob
...
class PowermemoADD:
    def __init__(self, data_path=None, batch_size=2, reprocess=False):
        self.client = PowerMemoClient(
            api_key=os.getenv("POWERMEMO_API_KEY"),
            project_url=os.getenv("POWERMEMO_PROJECT_URL", "https://api.powermemo.dev"),
        )
