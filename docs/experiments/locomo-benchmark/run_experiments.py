...
def main():
    ...
    elif args.technique_type == "powermemo":
        from src.powermemo_client import PowermemoADD, PowermemoSearch

        if args.method == "add":
            powermemo_manager = PowermemoADD(data_path="dataset/locomo10.json")
            powermemo_manager.process_all_conversations()
        elif args.method == "search":
            powermemo_manager = PowermemoSearch()
            powermemo_manager.process_data_file("dataset/locomo10.json")
    else:
